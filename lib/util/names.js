...
const includesSublist = (sublist) => {
    return pipe(intersection(sublist), complement(isEmpty));
};
const isIncludedIn = flip(includes);
const hasMatchingType = (typesToMatch) => {
    return where({ type: isIncludedIn(typesToMatch) });
};
const hasMatchingInterfaces = (interfacesToMatch) => {
    return where({ interfaces: includesSublist(interfacesToMatch) });
};
const hasMatchingModifier = (modifierToMatch) => {
    return where({ modifier: isIncludedIn(modifierToMatch) });
};
const filterTestCasesAndSuites = filter(hasMatchingType([TYPES.suite, TYPES.testCase]));

const nameLens = lensProp('name');
const mapNames = (fn) => {
    return map(over(nameLens, fn));
};
const extractNames = map(view(nameLens));
const addModifier = (modifier) => {
    return map(assoc('modifier', modifier));
};
...
const getNamesByTypeMemoized = memoizeWith((type, options) => {
    return JSON.stringify({ type, options });
}, getNamesByType);

export function getTestCaseNames(options) {
    return getNamesByTypeMemoized(TYPES.testCase, options);
}

export function getSuiteNames(options) {
