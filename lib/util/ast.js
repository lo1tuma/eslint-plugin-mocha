...
function createAstUtils(settings) {
    const additionalNames = getAddtionalNames(settings);

    function isDescribe(node, options = {}) {
        const { modifiers = [ 'skip', 'only' ], baseNames = true } = options;
        const { additionalSuiteNames, additionalSuiteModifiers } = additionalNames;
        const describeAliases = getSuiteNames({ baseNames, modifiers, additionalSuiteNames, additionalSuiteModifiers });
    ...
    function isTestCase(node, options = {}) {
        const { modifiers = [ 'skip', 'only' ], baseNames = true } = options;
        const { additionalTestCaseNames, additionalTestCaseModifiers } = additionalNames;
        const testCaseNames = getTestCaseNames({
            baseNames, modifiers, additionalTestCaseNames, additionalTestCaseModifiers
        });
