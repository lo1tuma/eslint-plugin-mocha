 ...
 */
/* eslint "complexity": [ "error", 18 ], "max-statements": [ "error", 15 ] -- the rule is copied from ESLint core and just slightly modified that’s why we accept those complexity issues for now */
...
export const preferArrowCallbackRule = {
    ...
    create(context) {
         ...
         *   {boolean} returnValue.isMochaCallback - `true` if the node is an argument to a mocha function.
                ...
                switch (parent.type) {
                    ...
                    case 'MemberExpression':
                        if (isBindThis(parent, currentNode)) {
                            returnValue.isLexicalThis = parent.parent.arguments.length === 1 &&
                    ...
                    case 'NewExpression':
                        ...
                        if (
                            returnValue.isCallback &&
                            isMochaFunctionCall(parent, context)
                        ) {
                            returnValue.isMochaCallback = true;
                        }
                        searchComplete = true;
                        break;
                ...
                if (
                    ...
                    context.report({
                        ...
                        fix(fixer) {
                            ...
                            const paramsLeftParen = node.params.length > 0
                            ...
                            const arrowFunctionText = `${asyncKeyword}${paramsFullText} => ${
                                sourceCode.getText(
                                    node.body
                                )
                            }`;
