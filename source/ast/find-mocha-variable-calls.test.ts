...
import {
    findMochaVariableCalls,
    splitCustomNamesByInterface
} from './find-mocha-variable-calls.js';
...
describe('findMochaVariableCalls()', function () {
    it('splitCustomNamesByInterface() keeps require names out of globals', function () {
        const result = splitCustomNamesByInterface([
            { interface: 'require' },
            { interface: 'BDD' }
        ] as never);

        assert.strictEqual(result.require.length, 1);
        assert.strictEqual(result.globals.length, 1);
        assert.strictEqual(result.require[0]?.interface, 'require');
        assert.strictEqual(result.globals[0]?.interface, 'BDD');
    });
    ...
    it('does not treat require-interface custom names as globals', function () {
        const calls = findCalls(
            'foo()',
