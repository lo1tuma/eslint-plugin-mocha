import { isConstantPath, type DynamicPath } from '../ast/member-expression.js';
import { filterWithArgs, flatMapWithArgs, mapWithArgs } from '../list.js';
import { configCallNames, type MochaConfigCall, type MochaInterface, type MochaEntityType, type NameDetailsConfig } from './descriptors.js';

function hasMatchingType(item: NameDetailsConfig, typesToMatch: readonly MochaEntityType[]): boolean {
...
function hasMatchingInterface(item: NameDetailsConfig, interfaceToMatch: MochaInterface): boolean {
    return interfaceToMatch === item.interface;
}

function filterByInterface(nameDetailsList: readonly NameDetailsConfig[], interfaceToMatch: MochaInterface): readonly NameDetailsConfig[] {
    return filterWithArgs(nameDetailsList, hasMatchingInterface, interfaceToMatch);
}

function formatXVariant(nameDetails: NameDetailsConfig): NameDetailsConfig {
...
function formatSkipVariant(nameDetails: NameDetailsConfig): NameDetailsConfig {
    return {
        ...nameDetails,
...
function formatExclusiveVariants(nameDetails: NameDetailsConfig): NameDetailsConfig {
    return {
        ...nameDetails,
...
function formatConfigVariant(nameDetails: NameDetails, config: MochaConfigCall): NameDetails {
    return {
        ...nameDetails,
        config,
        type: 'config',
...
function buildConfigVariantsForConfig(config: MochaConfigCall, nameDetailsList: readonly NameDetails[]): readonly NameDetails[] {
...
export function reformatLastPathSegmentWithCallExpressions(path: DynamicPath, amountOfCallExpressions: number): DynamicPath {
...
function normalizePathForNameDetails(nameDetails: NameDetailsConfig): NameDetails {
    return {
        ...nameDetails,
...
export function buildAllNameDetailsWithVariants(baseNameDetailsList: readonly NameDetailsConfig[]): readonly NameDetails[] {
